...
absolute_json="/share/work1/zhanglj/report_backup_wes/test_json/"
absolute_pdf="/share/work1/zhanglj/report_backup_wes/"
pdf_name=""


###提取真正的一代验证图片和wescnv图片
def get_verify_imgs_wescnv_imgs(dict):
	wes_verify_imgs={}
	wescnv_imgs_names=''
	num=1
...
#--------------获取科诺安cnv图片路径信息--------------------_#
def get_cnv_picture_result(cnv_picture_file=[],bus_code="",status=""): 
	#利用sftp获取CNV结果图
	if status.strip():
...
def get_all_qc_path(bus_code_list,config_path=os.path.split(os.path.realpath(__file__))[0]):
	...
	return result
###########获取扩展报告绝对路径
def get_absolute_extend_path(bus_code_list):
	extend_prefix="/share/production/Genetics/WES/"
	extend_path_list=[]
	result=""
	if len(bus_code_list):
		for bus_code in bus_code_list:
			...
			#print glob.glob(tmp_extend_path)
			##扩展绝对路径
		...
		if "," in temp:
			...
			if len(temp_data):
				...
				for item in temp_data:
					...
					'''if temp_sample in sample_code  and 'C0' in sample_code and temp_sample!=sample_code:
						continue'''
...
def  main():
	...
	if options.ana_type=="bulk" and options.check_result=="positive":
		...
		if len(exam_gd['supplement_report']):
			...
			exam_gd['supplement_sites']=trans_note_name(exam_gd['negative_appendix'])
